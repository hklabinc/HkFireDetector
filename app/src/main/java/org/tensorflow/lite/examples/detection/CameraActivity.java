...
public abstract class CameraActivity extends AppCompatActivity
    implements MqttCallback, OnImageAvailableListener,
...
//        CompoundButton.OnCheckedChangeListener,
  ...
  protected static final String LOGTAG = "[HHCHOI]";
  private static final String SERVER_ADDR = "ictrobot.hknu.ac.kr";
  private static final int    MQTT_PORT = 8085;
  private static final int    QOS    = 2;     // At most once (0), At least once (1), Exactly once (2)
  private static final String TOPIC_SUB = "HkPlatform/Camera/Control";
  protected static final String TOPIC_PUB_EVENT = "HkPlatform/Camera/Event";
  protected static final String TOPIC_PUB_IMAGE = "HkPlatform/Camera/Image";
  private MqttClient client;
  private MqttConnectOptions option;
  protected static String androidId = null;
  protected static String appName = null;
  protected static final float IMAGE_INTERVAL = 0.5f;
  protected static final int EVENT_INTERVAL = 3;
  protected static final int EVENT_THRESHOLD = 10;
  protected static final boolean isCropped = true;
  protected boolean isSendImage = false;
  ...
  protected void onCreate(final Bundle savedInstanceState) {
    ...
    Log.d(LOGTAG, "androidId: " + androidId);
    ...
    Log.d(LOGTAG, "appName: " + appName);

    /** MQTT 연결 */
    String serverAddr = "tcp://" + SERVER_ADDR + ":" + MQTT_PORT;
    String clientId = appName + "-" + androidId;    // ClientID는 중복이 되지 않도록 appName과 androidID의 조합으로 사용
    initialize(serverAddr, clientId);
    if(client != null) {
      subscribe(TOPIC_SUB);
    }
  }
...
//  }
  ...
  /** For MQTT */
  /* MQTT initialize */
  public void initialize(String serverAddr, String clientId) {
    option = new MqttConnectOptions();
    option.setCleanSession(true);
    option.setKeepAliveInterval(30);
    try {
      client = new MqttClient(serverAddr, clientId, new MemoryPersistence());        // clientId 써줘야! 안써주면 에러 발생!
      client.setCallback(this);
      client.connect(option);

      Log.d(LOGTAG, "MQTT initialize success!");
      Toast.makeText(getApplicationContext(), "MQTT 연결 성공", Toast.LENGTH_LONG).show();
    } catch (MqttException e) {
      Log.d(LOGTAG, "MQTT initialize fail!");
      Toast.makeText(getApplicationContext(), "MQTT 연결 실패", Toast.LENGTH_LONG).show();
      e.printStackTrace();
    }
  }
  ...
  public void publish(String topic_pub, String message) {
    try {
      ...
      Log.d(LOGTAG, "MQTT tx: " + topic_pub + ", " + message.length() + " bytes, " + message.substring(0, Math.min(100, message.length())));

      if (topic_pub.equals(TOPIC_PUB_EVENT))
        Toast.makeText(getApplicationContext(), "이벤트 전송", Toast.LENGTH_SHORT).show();
    } catch (MqttException e) {
      e.printStackTrace();
    }
  }
  ...
  public void close(){
    if(client != null){
      try {
        client.disconnect();
        Log.d(LOGTAG, "MQTT close success!!");
        client.close();
      } catch (MqttException e) {
        Log.d(LOGTAG, "MQTT close error!!");
        e.printStackTrace();
      }
    }
  }

  // MQTT 메시지 도착시 자동으로 호출됨
  @Override
  public void messageArrived(String topic, MqttMessage message) throws Exception {
    String rxMsg = message.toString();
    final String substring = rxMsg.substring(0, Math.min(100, rxMsg.length()));
    Log.d(LOGTAG, "MQTT rx: " + topic + ", " + rxMsg.length() + " bytes, " + substring);

    if (rxMsg.equals("Start")) {
      isSendImage = true;
    }
    else if (rxMsg.equals("Stop")) {
      isSendImage = false;
    }
    ...
    // Handler 사용 (UI 접근 위해)
      handler.post(new Runnable() {
          ...
          public void run() {
              ...
              Toast.makeText(getApplicationContext(), "MQTT rx (" + rxMsg.length() + "): " + substring, Toast.LENGTH_LONG).show();
          }
      });
  }
